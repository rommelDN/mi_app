...
from PySide6.QtWidgets import (
    ...
    QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QFrame, QSplitter, QRadioButton, QButtonGroup, QMessageBox
)
...
from services.homotencia import homotencia
...
class VentanaTransformaciones(QMainWindow):
    ...
    def __init__(self):
        ...
        # -----------------------------
        self.setWindowTitle("Proyecto Transformaciones Lineales")  # Título de la ventana
        ...
        self.btn_cubo.toggled.connect(lambda checked: self.toggle_radio(self.btn_cubo, checked))
        ...
        self.btn_reflexion.toggled.connect(lambda checked: self.toggle_radio(self.btn_reflexion, checked))
        ...
        # --- Homotecia ---
        layout_izq.addWidget(QLabel("Aplicar Homotecia a la Figura:"))
        ...
        self.btn_homotencia.toggled.connect(lambda checked: self.toggle_radio(self.btn_homotencia, checked))
        ...
        self.input_rx = QLineEdit(); self.input_rx.setPlaceholderText("Ángulo eje X")
        self.input_ry = QLineEdit(); self.input_ry.setPlaceholderText("Ángulo eje Y")
        self.input_rz = QLineEdit(); self.input_rz.setPlaceholderText("Ángulo eje Z")
    ...
    def dibujar_figura(self, button, checked):
        ...
        if not self.canvas.isVisible():
            QMessageBox.warning(self, "Advertencia", "La pestaña de 2D está minimizada, por favor expándala.")
    ...
    def dibujar_figura3D(self, button, checked):
        ...
        if not self.canvas3d.isVisible():
            QMessageBox.warning(self, "Advertencia", "La pestaña de 3D está minimizada, por favor expándala.")
            button.setChecked(False)
            return
        if button.text() == "Cubo":
    ...
    def aplicar_trasformacion(self):
        ...
        if has_3d:
            try:
                # Ángulos ingresados en cada eje (si están vacíos, se toma 0)
                ang_x = float(self.input_rx.text().strip()) if self.input_rx.text().strip() else 0.0
                ang_y = float(self.input_ry.text().strip()) if self.input_ry.text().strip() else 0.0
                ang_z = float(self.input_rz.text().strip()) if self.input_rz.text().strip() else 0.0
            except ValueError:
                return

            # Convertir vértices actuales en array numpy
            puntos3d = np.array(self.canvas3d.vertices if hasattr(self.canvas3d, 'vertices') else [], dtype=float)
            if puntos3d.size == 0:
                return
